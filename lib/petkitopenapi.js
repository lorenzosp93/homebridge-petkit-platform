...
class PetkitOpenAPI {
  ...
  constructor(username, password, countryCode, log) {
    ...
    this.endpoint = (this.countryCode)? new CountryUtil().getEndPointWithCountryCode(this.countryCode) : "http://api.eu-pet.com/latest"
    this.log = log;
  ...
  async get_assets() {
    let res = await this.post('/discovery/device_roster', {groupId: "0"});
